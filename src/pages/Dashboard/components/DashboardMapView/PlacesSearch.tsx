...
export function PlacesSearch({ onPlaceSelected }: PlacesSearchProps) {
  ...
  React.useEffect(() => {
    ...
    placeAutocomplete.className =
      'bg-white boder text-gray-700 block w-full [&>input]:w-full [&>input]:rounded-lg [&>input]:border [&>input]:border-gray-200 [&>input]:px-3 [&>input]:py-2 [&>input]:text-sm [&>input]:outline-none [&>input]:focus:ring-2 [&>input]:focus:ring-gray-300'
      ...
      <div className="rounded bg-white/95 shadow-lg ring-1 ring-black/5">
        <div className="relative">
          <div
            ...
            className="[&>gmp-place-autocomplete]:block [&>gmp-place-autocomplete]:w-full [&>gmp-place-autocomplete>input]:pl-8 bg-white text-gray-700"
          />
        </div>
      </div>
    </div>
